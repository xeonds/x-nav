      ...
      body: Column(
        children: [
          Consumer(builder: (context, ref, child) {
            final kvs = ref
                .watch(buildStreamProvider((db) => db.select(db.kVs).watch()));
            return kvs.when(
                data: (data) {
                  ...
                  return RideSummary(
                      totalDistance: dataMap['totalDistance'],
                      totalRides: dataMap['totalRides'],
                      totalTime: dataMap['totalTime']);
                },
                error: (s, e) => const Center(child: Text('加载失败')),
                loading: () =>
          ...
          Expanded(child: Consumer(builder: (context, ref, child) {
            ...
            // 获取某条历史记录的 summary 数据
            summaryOfHistory(int id) =>
                ref.watch(buildStreamProvider((db) => (db.select(db.summarys)
                      ..where((tbl) => tbl.historyId.equals(id)))
                    .watch()
                                        ...
                                        onPressed: () async {
                                          ...
                                          if (confirm == true) {
                                            for (var index in selectedIndices) {
                                              await File(data[index].filePath)
                                                  .delete();
                                            }
                                        ...
                                        child: Container(
                                          ...
                                          child: Consumer(
                                              builder: (context, ref, child) =>
                                                  summaryOfHistory(data[index].summaryId ?? 0).when(
                                                      data: (res) =>
  ...
  static Widget buildRideHistoryCard(
    ...
    return Card(
      child: ListTile(
        ...
        title: Text(
            '${summary.startTime!.toLocal().toString().replaceFirst('T', ' ')} 的骑行'),
        subtitle: Column(
