...
const extractTCXTracks = (tcx, name) => {
    if (!tcx.Activities) {
...
const extractFITTracks = (fit, name) => {
...
const readFile=(file, encoding, isGzipped)=> {
...
export const extractTracks = (file)=> {
    const isGzipped = /\.gz$/i.test(file.name);
    const strippedName = file.name.replace(/\.gz$/i, '');
    const format = strippedName.split('.').splice(-1)[0].toLowerCase();
